var graph_filler = function(data,graph) {
  ...
  for (i in data) {
    ...
    for (u in data[i].contributor) {
      if (data[i].contributor[u].name) { continue; }
      var name = data[i].contributor[u].replace(/^.*\//,"")
      ...
      if (name_id[name] == null) {
        name_n = name_n + 1;
        name_id[name] = name_n;
        graph.nodes.push({"name":name, "group":expertise_id[expertise], "link":"http://software.esciencecenter.nl/person/" + name, "type":"person"})
      }
      graph.links.push({
